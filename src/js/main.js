...
var SPIRIT_VISAGE = 'Spirit Visage';
var RYLAIS_CRYSTAL_SCEPTER = 'Rylai\'s Crystal Scepter';
...
// Scale Values
var SCALE_CHIMES_PER_MEEP = 1;
var SCALE_CHIMES_EXPERIENCE = 0.9;
var SCALE_ITEM_COST = 0.1;
var SCALE_MONSTER_REWARD = 0;
var SCALE_MONSTER_HEALTH = 0.02;
var SCALE_MONSTER_LEVEL_REWARD = 5;
var SCALE_MONSTER_LEVEL_HEALTH = {
  'easy': 4.5,
  'medium': 5.0,
  'hard': 5.6,
  'marathon': 6.6,
...
function itemToIndex(item) {
  return isString(item) ? ITEM_TO_INDEX[item] : item;
}
function upgradeToIndex(upgrade) {
  return isString(upgrade) ? UPGRADE_TO_INDEX[upgrade] : upgrade;
}
function spellToIndex(spell) {
  return isString(spell) ? SPELL_TO_INDEX[spell] : spell;
}
function monsterToIndex(monster) {
  return isString(monster) ? MONSTER_TO_INDEX[monster] : monster;
}
function typeToIndex(type) {
  return isString(type) ? TYPE_TO_INDEX[type] : type;
}
function runeTypeToIndex(runeType) {
  return isString(runeType) ? RUNE_TYPE_TO_INDEX[runeType] : runeType;
}
function runeToIndex(rune) {
  return isString(rune) ? RUNE_TO_INDEX[rune] : rune;
}
function indexToItem(index) {
  return isString(index) ? index : INDEX_TO_ITEM[index];
}
function indexToUpgrade(index) {
  return isString(index) ? index : INDEX_TO_UPGRADE[index];
}
function indexToSpell(index) {
  return isString(index) ? index : INDEX_TO_SPELL[index];
}
function indexToMonster(index) {
  return isString(index) ? index : INDEX_TO_MONSTER[index];
}
function indexToType(index) {
  return isString(index) ? index : INDEX_TO_TYPE[index];
}
function indexToRuneType(index) {
  return isString(index) ? index : INDEX_TO_RUNE_TYPE[index];
}
function indexToRune(index) {
  return isString(index) ? index : INDEX_TO_RUNE[index];
}
...
function showRing(spellName, duration) {
  var id;
  if (spellName == SPOILS_OF_WAR)
    id = '#spoils-ring';
  else if (spellName == TRIBUTE)
    id = '#tribute-ring';
  else if (spellName == SMITE)
    id = '#smite-ring';
  else if (spellName == CHALLENGING_SMITE)
...
GameApp.controller('GameController', ['$scope', '$sce', function ($scope, $sce) {
  ...
  $scope.prettyTime = function (seconds) {
  ...
  $scope.itemToIndex = function (a) {
    return itemToIndex(a);
  };
  $scope.upgradeToIndex = function (a) {
    return upgradeToIndex(a);
  };
  $scope.spellToIndex = function (a) {
    return spellToIndex(a);
  };
  $scope.monsterToIndex = function (a) {
    return monsterToIndex(a);
  };
  $scope.typeToIndex = function (a) {
    return typeToIndex(a);
  };
  $scope.indexToItem = function (a) {
    return indexToItem(a);
  };
  $scope.indexToUpgrade = function (a) {
    return indexToUpgrade(a);
  };
  $scope.indexToSpell = function (a) {
    return indexToSpell(a);
  };
  $scope.indexToMonster = function (a) {
    return indexToMonster(a);
  };
  $scope.indexToType = function (a) {
    return indexToType(a);
  };
  $scope.removeRune = function (game, type, n) {
    game.removeRune(game[type][n]);
  };
  $scope.trustHtml = function (s) {
    return $sce.trustAsHtml(s);
  };
  $scope.range = function (n) {
    return new Array(n);
  };
  $scope.showAvailableRunes = function (t) {
    return showAvailableRunes(t);
  };
  $scope.zeroClass = function (n) {
    return !n ? 'zero' : '';
  };
  $scope.negativeClass = function (n) {
    return n < 0 ? 'negative' : '';
  };
  $scope.changedClass = function (game, stat) {
    if (game.runeStats[stat] > game.tempRuneStats[stat])
      return 'negative';
    else if (game.runeStats[stat] < game.tempRuneStats[stat])
      return 'positive';
    else
      return '';
  };
...
function initializeHotkeys(game) {
  ...
  $(document).bind('keydown.d', function () {
    $('#chimes-bar-dropdown .dropdown-button').click();
  });
  $(document).bind('keydown.f', function () {
    $('#damage-bar-dropdown .dropdown-button').click();
  });
  // Dialog hotkeys
  ...
  $(document).bind('keydown.alt_n', function () {
    game.showNewGameModal(false);
  });
  ...
  $(document).bind('keydown.1', function () {
    game.buyItem(RELIC_SHIELD);
  });
  $(document).bind('keydown.2', function () {
    game.buyItem(ANCIENT_COIN);
  });
  $(document).bind('keydown.3', function () {
    game.buyItem(SPELLTHIEFS_EDGE);
  });
  $(document).bind('keydown.4', function () {
    game.buyItem(BOOTS_OF_SPEED);
  });
  $(document).bind('keydown.5', function () {
    game.buyItem(RUBY_CRYSTAL);
  });
  $(document).bind('keydown.6', function () {
    game.buyItem(AMPLIFYING_TOME);
  });
  $(document).bind('keydown.7', function () {
    game.buyItem(DAGGER);
  });
  $(document).bind('keydown.ctrl_1', function () {
    game.buyItem(RELIC_SHIELD, 10);
    return false;
  });
  $(document).bind('keydown.ctrl_2', function () {
    game.buyItem(ANCIENT_COIN, 10);
    return false;
  });
  $(document).bind('keydown.ctrl_3', function () {
    game.buyItem(SPELLTHIEFS_EDGE, 10);
    return false;
  });
  $(document).bind('keydown.ctrl_4', function () {
    game.buyItem(BOOTS_OF_SPEED, 10);
    return false;
  });
  $(document).bind('keydown.ctrl_5', function () {
    game.buyItem(RUBY_CRYSTAL, 10);
    return false;
  });
  $(document).bind('keydown.ctrl_6', function () {
    game.buyItem(AMPLIFYING_TOME, 10);
    return false;
  });
  $(document).bind('keydown.ctrl_7', function () {
    game.buyItem(DAGGER, 10);
    return false;
  });
  $(document).bind('keydown.shift_1', function () {
    game.buyItem(RELIC_SHIELD, 100);
  });
  $(document).bind('keydown.shift_2', function () {
    game.buyItem(ANCIENT_COIN, 100);
  });
  $(document).bind('keydown.shift_3', function () {
    game.buyItem(SPELLTHIEFS_EDGE, 100);
  });
  $(document).bind('keydown.shift_4', function () {
    game.buyItem(BOOTS_OF_SPEED, 100);
  });
  $(document).bind('keydown.shift_5', function () {
    game.buyItem(RUBY_CRYSTAL, 100);
  });
  $(document).bind('keydown.shift_6', function () {
    game.buyItem(AMPLIFYING_TOME, 100);
  });
  $(document).bind('keydown.shift_7', function () {
    game.buyItem(DAGGER, 100);
  });
  $(document).bind('keydown.ctrl_shift_1', function () {
    game.buyItem(RELIC_SHIELD, 1000);
  });
  $(document).bind('keydown.ctrl_shift_2', function () {
    game.buyItem(ANCIENT_COIN, 1000);
  });
  $(document).bind('keydown.ctrl_shift_3', function () {
    game.buyItem(SPELLTHIEFS_EDGE, 1000);
  });
  $(document).bind('keydown.ctrl_shift_4', function () {
    game.buyItem(BOOTS_OF_SPEED, 1000);
  });
  $(document).bind('keydown.ctrl_shift_5', function () {
    game.buyItem(RUBY_CRYSTAL, 1000);
  });
  $(document).bind('keydown.ctrl_shift_6', function () {
    game.buyItem(AMPLIFYING_TOME, 1000);
  });
  $(document).bind('keydown.ctrl_shift_7', function () {
    game.buyItem(DAGGER, 1000);
  });
  $(document).bind('keydown.alt_ctrl_shift_1', function () {
    game.buyItem(RELIC_SHIELD, 10000);
  });
  $(document).bind('keydown.alt_ctrl_shift_2', function () {
    game.buyItem(ANCIENT_COIN, 10000);
  });
  $(document).bind('keydown.alt_ctrl_shift_3', function () {
    game.buyItem(SPELLTHIEFS_EDGE, 10000);
  });
  $(document).bind('keydown.alt_ctrl_shift_4', function () {
    game.buyItem(BOOTS_OF_SPEED, 10000);
  });
  $(document).bind('keydown.alt_ctrl_shift_5', function () {
    game.buyItem(RUBY_CRYSTAL, 10000);
  });
  $(document).bind('keydown.alt_ctrl_shift_6', function () {
    game.buyItem(AMPLIFYING_TOME, 10000);
  });
  $(document).bind('keydown.alt_ctrl_shift_7', function () {
    game.buyItem(DAGGER, 10000);
  });
  $(document).bind('keydown.alt_1', function () {
    var upgrade = game.items[RELIC_SHIELD].upgradesAvailable[0];
    if (upgrade) {
      game.buyUpgrade(upgrade);
    }
  });
  $(document).bind('keydown.alt_2', function () {
    var upgrade = game.items[ANCIENT_COIN].upgradesAvailable[0];
    if (upgrade) {
      game.buyUpgrade(upgrade);
    }
  });
  $(document).bind('keydown.alt_3', function () {
    var upgrade = game.items[SPELLTHIEFS_EDGE].upgradesAvailable[0];
    if (upgrade) {
      game.buyUpgrade(upgrade);
    }
  });
  $(document).bind('keydown.alt_4', function () {
    var upgrade = game.items[BOOTS_OF_SPEED].upgradesAvailable[0];
    if (upgrade) {
      game.buyUpgrade(upgrade);
    }
  });
  $(document).bind('keydown.alt_5', function () {
    var upgrade = game.items[RUBY_CRYSTAL].upgradesAvailable[0];
    if (upgrade) {
      game.buyUpgrade(upgrade);
    }
  });
  $(document).bind('keydown.alt_6', function () {
    var upgrade = game.items[AMPLIFYING_TOME].upgradesAvailable[0];
    if (upgrade) {
      game.buyUpgrade(upgrade);
    }
  });
  $(document).bind('keydown.alt_7', function () {
...
$(window).load(function () {
  ...
  $('body').click(function () {
    if (activeModal == 'difficulty' || activeModal == 'menu')
      hideModal();
  });
...
function showModal(modal, args) {
  ...
  if (modal == 'stats')
    ...
    showDifficultyModal(args);
  else if (modal == 'newgame')
    showNewGameModal(args);
  else if (modal == 'win')
    showWinModal(args);
  else if (modal == 'export')
    showExportModal(args);
...
function hideSubpage(onEnd, page) {
  ...
  $('#subpage-panel iframe').animate({ opacity: 0 }, 300, function () {
