...
Spell.Create = function(game) {
  ...
  spells[GHOST] = new Spell(game, 10, 90, SPELL_ACTIVE, MONSTER_ALL,
    ...
    "+100% chime gathering for 10 seconds.  </br></br>" + Math.round(this.cooldown) + " second cooldown. <b>(Q)</b>"}
  );

  spells[FLASH] = new Spell(game, 0, 180, SPELL_ACTIVE, MONSTER_ALL,
  ...
  spells[EXHAUST] = new Spell(game, 10, 90, SPELL_ACTIVE, MONSTER_CHAMPION,
    ...
    function(game) {return this.status == LOCKED ? "":
    "+100% damage dealt for 10 seconds.  Only works against champions.  </br></br>" + Math.round(this.cooldown) + " second cooldown. <b>(T)</b>"}
  );

  spells[IGNITE] = new Spell(game, 0, 120, SPELL_ACTIVE, MONSTER_ALL,
    function(game) {game.igniteBonus += .03;
                    game.updateStats();
  ...
  spells[TELEPORT] = new Spell(game, 0, 300, SPELL_ACTIVE, MONSTER_ALL,
    function(game) {},
    function(game) {var cooldownSpells = game.getObjectsByStatus(game.spells, COOLDOWN);
    ...
    function(game) {return this.status == LOCKED ? "":
    "Reset cooldowns of all spells.  </br></br>" + Math.round(this.cooldown) + " second cooldown. <b>(Y)</b>"}
  );

  spells[SPOILS_OF_WAR] = new Spell(game, 0, 45, SPELL_PASSIVE, MONSTER_JUNGLE,
    function(game) {game.spoilsOfWarActive = 1;
                    game.killMonster();
                    game.spoilsOfWarActive = 0;
                    showRing(SPOILS_OF_WAR, RING_DURATION)},
    function(game) {},
    function(game) {return game.upgrades[FACE_OF_THE_MOUNTAIN].status == PURCHASED;},
    function(game) {return this.status == LOCKED ? "":
    "Execute monsters below 25% max health on click, gaining <b>+" + (game.spoilsOfWarBonus * 100).toFixed(1) + "%</b> reward gold.  Gold scales with Relic Shields owned.  Does not work against champions. </br></br>" + Math.round(this.cooldown) + " second cooldown."}
  );

  spells[FAVOR] = new Spell(game, 0, 0, SPELL_PASSIVE, MONSTER_ALL,
    function(game) {},
    function(game) {},
    function(game) {return game.upgrades[TALISMAN_OF_ASCENSION].status == PURCHASED;},
    function(game) {return game.spells[FAVOR].status == LOCKED ? "":
  ...
  spells[TRIBUTE] = new Spell(game, 0, 30, SPELL_PASSIVE, MONSTER_ALL,
